...
export default function Musicas() {
	...
	const { currentTime, duration, isPlaying, togglePlay, handleSeek } = useYouTubeAudioPlayer(song?.videoId ?? "");
				...
				<div className="h-full w-full p-5 flex flex-col gap-3 rounded-3xl bg-gray-800">
					<div className="w-full grid place-items-center">
						<div className="w-full flex items-center justify-between">
							...
							<h2 className="w-full text-center text-3xl font-bold text-gray-50">{song?.title}</h2>
							<div className="w-[32px]" />
						</div>
						<h3 className="text-xl font-semibold text-gray-100">{formattedVoice}</h3>
					</div>
					<div className="h-[60vh] w-full flex flex-col gap-3">
						{song && <TextReader lyrics={song.lyrics} currentTime={currentTime} />}
					</div>
				</div>
